...
export default {
  ...
  computed: {
    ...
    isCurrentAudioAFileAudio() {
      return (
        this.currentAudio.source_format === "file" ||
        this.currentAudio.source_format === "audio_url"
      );
    },
    ...
    isOwned() {
      return (
        (this.currentAudio.artist &&
          ...
          this.currentAudio.artist.id ===
            this.$store.getters.getUser.artist.id) ||
        (this.currentAudio.product && this.isPurchased(this.currentAudio))
      );
    },
    getPlayerStatus() {
    ...
    fetchCampaigns() {
      ...
      const getCampaigns = () => {
        axios.get("/api/campaigns").then((res) => {
          if (JSON.stringify(this.campaigns) !== JSON.stringify(res.data)) {
            this.campaigns = res.data;
            this.prepareCampaign(this.currentCampaignIndex);
          }
        });
      };
      if (user && user.id) {
