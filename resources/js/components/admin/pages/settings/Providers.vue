    ...
    <v-card class="page" v-if="!isLoading">
        ...
        <v-container fluid>
            <v-row>
                <v-col cols="12">
                    <v-switch
                        v-model="settings.provider_spotify"
                        label="Spotify API"
                    ></v-switch>
                </v-col>
                <template v-if="settings.provider_spotify">
                    ...
                    <v-col cols="6">
                        <v-text-field
                            ...
                            hide-details=""
                            outlined
                            v-model="settings.spotifyClientSecret"
                            label="Spotify Client Secret"
                        ></v-text-field>
                    </v-col>
                    <v-col cols="12">
                        <v-switch
                            v-model="settings.spotify_search"
                            :label="$t('Search')"
                            :messages="
                                $t(
                                    'Includes search results from Spotify engine ( search bar of the player )'
                                )
                            "
                        ></v-switch>
                    </v-col>
                    <v-col cols="12">
                        <v-switch
                            v-model="settings.show_external_link_spotify"
                            :label="$t('Show Spotify external links')"
                            :messages="
                                $t(
                                    'A Button shows on Spotify content pages that takes to the official page'
                                )
                            "
                        ></v-switch>
                    </v-col>
...
export default {
    ...
    methods: {
        save() {
            ...
            formData.append(
                "provider_spotify",
                this.settings.provider_spotify ? 1 : 0
            );
            formData.append(
                "provider_listenNotes",
                this.settings.provider_listenNotes ? 1 : 0
            );
            formData.append(
                "show_external_link_spotify",
                this.settings.show_external_link_spotify ? 1 : 0
            );
            formData.append(
                "spotify_search",
                this.settings.spotify_search ? 1 : 0
            );
            formData.append(
                "listenNotes_search",
                this.settings.listenNotes_search ? 1 : 0
            );

            formData.append("spotifyClientID", this.settings.spotifyClientID);
            formData.append(
                "listenNotesClientID",
                this.settings.listenNotesClientID
            );
            formData.append(
                "spotifyClientSecret",
                this.settings.spotifyClientSecret
