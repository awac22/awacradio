...
export default {
    methods: {
        ...
        getMetaDataUsingProxy(stationID) {
            axios
                .get("/api/get-stream-metadata-pr/" + stationID)
                .then((res) => {
        ...
        getMetaDataFromServer(stationID) {
            axios
                .get("/api/get-stream-metadata-sr/" + stationID, {})
                .then((res) => {
        ...
        startFetchMetadata(station) {
            if (station.statsSource === "endpoint") {
                this.metadataFetcher = setInterval(
                    function x() {
                        this.getMetaDataUsingProxy(station.id);
                    }.bind(this),
                    station.interval || 5000
                );
            } else {
                this.metadataFetcher = setInterval(
                    function x() {
        ...
        initStreamPlayer(station) {
            if (station.statsSource === "endpoint" && !station.proxy) {
                ...
                this.isLoading = true;
                var timeInterval = 0;
                this.streamStats = new IcecastMetadataStats(
                    station.streamEndpoint,
                    {
                        ...
                        onPlay: () => {
                            this.interval = setInterval(() => {
                                ...
                                this.currentAudio.currentTime =
                                    this.getTimeFormat(timeInterval.toFixed(0));
                            }, 1000);
                ...
                this.updateAudioElement(
                    false,
                    this.currentAudio.streamEndpoint
                ).then(() => {
                    this.playPause();
                });
            }
