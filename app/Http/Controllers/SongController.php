...
class SongController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->artists) && isset($request->genres)) {
            $GenreArtist = new GenreArtist();
            foreach (json_decode($request->artists) as $artist) {
                foreach (json_decode($request->genres) as $genre) {

                    $GenreArtist->artist_id = $artist->id;
                    $GenreArtist->genre_id = $genre->id;
                    $GenreArtist->song_id = $song->id;
    ...
    public function update(Request $request, $id)
    {
        ...
        \DB::table('genre_artist')->where('song_id', $song->id)->delete();
        if (isset($request->artists) && isset($request->genres)) {
            foreach (json_decode($request->artists) as $artist) {
                foreach (json_decode($request->genres) as $genre) {
                    DB::table('genre_artist')->insert([
                        'artist_id' => $artist->id,
                        'genre_id' => $genre->id,
                        'song_id' => $song->id
                    ]);
